...
effort. Since this is the wild west, with dreadful weather and not much water, it's important 
...
Notes
Not all paths can be made simpler. The path ["NORTH"
*/
internal static class Task_8
{
    public static string[] dirReduc(String[] arr)
    {
        ...
        while (true)
        {
            ...
            {
                if (list[i] == "NORTH" && list[i + 1] == "SOUTH" || list[i] == "SOUTH" && list[i + 1] == "NORTH"
                    || list[i] == "EAST" && list[i + 1] == "WEST" || list[i] == "WEST" && list[i + 1] == "EAST")
                {
                    list[i] = "0";
                    list[i + 1] = "0";
                }
